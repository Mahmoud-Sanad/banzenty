        ...
        <form method="POST" action="{{ route("admin.stations.update", [$station->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <div class="d-flex">
                    <input class="form-control col-md-6 {{ $errors->has('name') ? 'is-invalid' : '' }}"
                        type="text" name="name[en]" id="name" value="{{ old('name.en', $station->getTranslation('name', 'en')) }}"
                        placeholder="English">
                    <input class="form-control col-md-6 {{ $errors->has('name') ? 'is-invalid' : '' }}"
                        type="text" name="name[ar]" id="name" value="{{ old('name.ar', $station->getTranslation('name', 'ar')) }}"
                        placeholder="عربي">
                </div>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}" name="company_id" id="company_id" required>
                    @foreach($companies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('company_id') ? old('company_id') : $station->company->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lat') || $errors->has('lng') ? 'is-invalid' : '' }}"
                    type="text" name="location" id="location" value="{{ old('location', $station->lat .','. $station->lng) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" type="text" name="address" id="address" value="{{ old('address', $station->address) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('has_contract') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="has_contract" id="has_contract" value="1" {{ $station->has_contract || old('has_contract', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('services') ? 'is-invalid' : '' }}" name="services[]" id="services" multiple>
                    @foreach($services as $id => $service)
                        <option value="{{ $id }}" {{ (in_array($id, old('services', [])) || $station->services->contains($id)) ? 'selected' : '' }}>{{ $service }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fuels') ? 'is-invalid' : '' }}" name="fuels[]" id="fuels" multiple required>
                    @foreach($fuels as $id => $fuel)
                        <option value="{{ $id }}" {{ (in_array($id, old('fuels', [])) || $station->fuels->contains($id)) ? 'selected' : '' }}>{{ $fuel }}</option>
                    @endforeach
                </select>
                @if($errors->has('fuels'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('users') ? 'is-invalid' : '' }}" name="users[]" id="users" multiple>
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ (in_array($id, old('users', [])) || $station->users->contains($id)) ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('users'))
                    <div class="invalid-feedback">
...
@section('scripts')
    ...
    <script>
        $('#location').leafletLocationPicker({
            alwaysOpen: true,
            mapContainer: "#map",
            location: "{{ old('location', $station->lat .','. $station->lng) }}",
            height: 300,
            layer: 'JAWG',
