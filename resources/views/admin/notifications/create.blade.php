    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.notifications.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.notification.fields.title') }}</label>
                <div class="d-flex">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.notification.fields.body') }}</label>
                <div class="d-flex">
                    <textarea class="form-control col-md-6 {{ $errors->has('body') ? 'is-invalid' : '' }}"
                        type="text" name="body[en]" placeholder="English" required
                        >{{ old('body.en', '') }}</textarea>
                    <textarea class="form-control col-md-6 {{ $errors->has('body') ? 'is-invalid' : '' }}"
                        type="text" name="body[ar]" placeholder="عربي" required
                        >{{ old('body.ar', '') }}</textarea>
                </div>
                @if($errors->has('body'))
                    <div class="invalid-feedback">
                        {{ $errors->first('body') }}
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('schedule') ? 'is-invalid' : '' }}" type="text" name="schedule" id="schedule" value="{{ old('schedule') }}">
