    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.companies.update", [$company->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.company.fields.name') }}</label>
                <div class="d-flex">
                    <input class="form-control col-md-6 {{ $errors->has('name') ? 'is-invalid' : '' }}"
                        type="text" name="name[en]" id="name" value="{{ old('name.en', $company->getTranslation('name', 'en')) }}"
                        placeholder="English" required>
                    <input class="form-control col-md-6 {{ $errors->has('name') ? 'is-invalid' : '' }}"
                        type="text" name="name[ar]" id="name" value="{{ old('name.ar', $company->getTranslation('name', 'ar')) }}"
            ...
            <div class="form-group">
                <label class="required" for="icon">{{ trans('cruds.company.fields.icon') }}</label>
