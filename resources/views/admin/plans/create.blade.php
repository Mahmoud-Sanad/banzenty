...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.plans.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="litres">{{ trans('cruds.plan.fields.litres') }}</label>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('period') ? 'is-invalid' : '' }}" name="period" id="period" required>
                    <option value disabled {{ old('period', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Plan::PERIOD_SELECT as $key => $label)
            ...
            <div class="form-group" id="services">
                ...
                <template id="service-template">
                    <div class="d-flex service">
                        {{ Form::select('[id]', $services , null, ['class' => 'form-control col-md-4 m-1 id']) }}
                        {{ Form::number('[discount]', null, ['class' => 'form-control col-md-3 m-1 discount', 'placeholder' => trans('global.discount_percent'), 'min' => 0, 'max' => 100]) }}
...
@javascript('services', old('services', [['id' => null, 'discount' => 0, 'limit' => null]]));
