    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="user_id">{{ trans('cruds.car.fields.plate_number') }}</label>
                <div class="row" style="max-width: 24rem">
                    <div class="col-3 pr-0">
                      <input name="plate_number_digits" type="number" placeholder="{{trans('cruds.car.fields.digits')}}"
                        class="form-control rounded-pill plate" id="digits" value="{{old('plate_number_digits', $user?->car?->digits)}}">
                    </div>
                    <div class="col-3 pr-0">
                      <input name="plate_number_characters" type="text" placeholder="{{trans('cruds.car.fields.characters')}}"
                        class="form-control rounded-pill plate" id="characters" value="{{old('plate_number_characters', $user?->car?->characters)}}">
                    </div>
                    <img src="{{url('images/loading.gif')}}" id="loading" class="img-fluid mx-3" style="display: none; height: calc(1.5em + .75rem + 2px)">
                    <div class="col-6 h5 my-auto" id="user_label">{{request()->has('user') ? ($user?->name ?? trans('global.user_not_found')) : ''}}</div>
                </div>
                <input type="hidden" name="user_id" value="{{old('user_id', $user?->id)}}" id="user_id" required>
                @if($errors->has('user'))
            ...
            <div class="form-group" style="display: none">
                ...
                {{ Form::select('fuel_id', [], old('fuel_id'), ['class' => 'form-control select2', 'id' => 'fuel_id']) }}
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('litres') ? 'is-invalid' : '' }}" type="number" name="litres" id="litres" value="{{ old('litres', '') }}" step="0.01" min="1">
