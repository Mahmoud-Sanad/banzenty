                ...
                <div class="row">
                    @foreach(App\Models\Banner::TARGET_TYPES as $class => $type)
                        <div class="col-md-3 d-flex">
                            {{ Form::radio('target_type', $class, old('target_type', $banner->target_type) == $class, ['class' => 'radio-input']) }}
                            <div class="mx-1">{{trans($type)}}</div>
                        </div>
                    @endforeach
                </div>
            </div>
...
<script>
    ...
    $(function() {
        if ($('[name="target_type"]:checked').val()) {
            updateIds("{{old('target_id', $banner->target_id)}}");
        }
    });
</script>
@endsection
